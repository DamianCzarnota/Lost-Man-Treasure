...
int main(void){
    ...
    while(1){
        znak =getch();
        switch(znak){
            case 'w': game->zm=UP; break;
            case 'd': game->zm=RIGHT; break;
            case 's': game->zm=DOWN; break;
            case 'a': game->zm=LEFT; break;
            case 'q': game->zm=QUIT; break;
            default: game->zm=STAY; break;
        }
        sem_post(sem_tab[1]);
